...
def judge(a, b, rounds)
  matches = 0
  rounds.times do
    matches += 1 if lowest(a.next) == lowest(b.next)
  end
  matches
end

input = File.open('15.txt', &:read).scan(/\d+/).map(&:to_i)

def easy(input, rounds)
  a = make_generator(input[0], 16_807)
  b = make_generator(input[1], 48_271)
  judge(a, b, rounds)
end

assert(easy([65, 8_921], 5) == 1)
assert(easy([65, 8_921], 40_000_000) == 588)
puts "easy(input, 40_000_000): #{easy(input, 40_000_000)}"
...
# values from each generator are compared, then the second values from
# each generator, then the third values, and so on.
...
def hard(input, rounds)
  a = make_picky_generator(input[0], 16_807, 4)
  b = make_picky_generator(input[1], 48_271, 8)
  judge(a, b, rounds)
end

assert(hard([65, 8_921], 1056) == 1)
puts "hard(input, 5_000_000): #{hard(input, 5_000_000)}"

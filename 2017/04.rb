...
input = File.open('04.txt') { |f| f.readlines.map(&:split) }

def passphrase_doubled?(passphrase)
  seen = Set.new
  passphrase.each do |word|
    return true if seen.include?(word)
    seen << word
  end
  false
end

def easy(passphrases)
  passphrases.count { |passphrase| !passphrase_doubled?(passphrase) }
end

test_input = [%w(aa bb cc dd ee), %w(aa bb cc dd aa), %w(aa bb cc dd aaa)]

assert(easy(test_input) == 2)
...
def passphrase_anagram?(passphrase)
  seen = Set.new
  passphrase.each do |word|
    candidate = frequencies(word)
    return true if seen.include?(candidate)
    seen << candidate
  end
  false
end

def hard(passphrases)
  passphrases.count { |passphrase| !passphrase_anagram?(passphrase) }
end

assert(!passphrase_anagram?(%w(abcde fghij)))
assert(passphrase_anagram?(%w(abcde xyz ecdab)))
assert(!passphrase_anagram?(%w(a ab abc abd abf abj)))
assert(!passphrase_anagram?(%w(iiii oiii ooii oooi oooo)))
assert(passphrase_anagram?(%w(oiii ioii iioi iiio)))

...
# building at regular intervals and descend down into the
...
# different types, and only matching types can be connected. You take
# an inventory of the components by their port types (your puzzle
...
def bridge_each(components, starter, result = nil, value = 0, &block)
  pool = components.clone.delete(starter)
  result ||= [starter]
  yield(result)
  other = starter[0] == value ? starter[1] : starter[0]
  pool.each do |link|
    next unless link.include?(other)
    bridge_each(pool, link, result + [link], other, &block)
  end
end
...
test_input = "0/2\n2/2\n2/3\n3/4\n3/5\n0/1\n10/1\n9/10"
input = File.open('24.txt') { |f| f.read.chomp }

assert(easy(test_input) == 31)
...
def hard(input)
  longest = longest_bridges(input)
  longest.map { |bridge| bridge_strength(bridge) }.max
end

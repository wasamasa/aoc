...
class Spinlock
  ...
  def step
    @index = (@index + @steps) % @positions.length + 1
    @value += 1
...
def easy(input)
  ...
  2017.times { spinlock.step }
  spinlock.peek
end

assert(easy(test_input) == 638)
...
class AngrySpinlock
  ...
  def step
    @step_size += 1
    @index = (@index + @steps) % @step_size + 1
    @value = @step_size if @index == 1
  end
...
def hard(input)
  spinlock = AngrySpinlock.new(input)
  50_000_000.times do |i|
    puts "#{i}/50000000" if (i % 1_000_000).zero?
    spinlock.step
  end
  spinlock.peek
end

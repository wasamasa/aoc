...
input = File.open('05.txt') { |f| f.readlines.map(&:chomp) }

def enough_vowels?(input)
  input.count('aeiou') >= 3
end

def doubled_letter?(input)
  input[/(.)\1/]
end

FORBIDDEN_STRINGS = %w[ab cd pq xy].freeze

def forbidden_strings?(input)
  FORBIDDEN_STRINGS.any? { |s| input.include?(s) }
end

def nice?(input)
  enough_vowels?(input) && doubled_letter?(input) && !forbidden_strings?(input)
end
...
#   in the string without overlapping, like `xyxy` (`xy`) or
...
def doubled_pair?(input)
  input[/(..).*?\1/]
end

def repeated_around?(input)
  input[/(.).\1/]
end

def nicer?(input)
  doubled_pair?(input) && repeated_around?(input)
end

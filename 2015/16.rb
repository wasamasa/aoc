...
def parse(line)
  id, _, attrs = line.partition(': ')
  a = attrs.split(', ').map do |attr|
    k, v = attr.split(': ')
    [k, v.to_i]
  end
  [id.split(' ')[1].to_i, a.to_h]
end

input = File.open('16.txt') { |f| f.readlines.map { |line| parse(line) }.to_h }
hints = { 'children' => 3,
          'cats' => 7,
          'samoyeds' => 2,
          'pomeranians' => 3,
...
def easy(input, hints)
  hints.each do |name, count|
    input = input.reject { |_, v| v[name] && v[name] != count }
  end
  assert(input.length == 1)
  input.first[0]
end

puts "easy(input, hints): #{easy(input, hints)}"
...
def hard(input, hints)
  hints.each do |name, count, op|
    input = input.select { |_, v| !v[name] || v[name].send(op, count) }
  end
  assert(input.length == 1)
  input.first[0]
end

puts "hard(input, better_hints): #{hard(input, better_hints)}"

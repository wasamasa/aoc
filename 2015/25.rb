...
# manual. Apparently, it refuses to run unless you give it that
...
# of the manuals. I bet we can find you the code generation algorithm,
# though."
...
input = File.open('25.txt') { |f| f.read.scan(/\d+/).map(&:to_i) }
...
def easy(input)
  code = 20_151_125
  cantor.each do |row_col|
    return code if row_col == input
    code *= 252_533
    code %= 33_554_393
  end
end

assert(easy([1, 1]) == 20_151_125)
assert(easy([2, 1]) == 31_916_031)
assert(easy([3, 1]) == 16_080_970)
assert(easy([1, 3]) == 17_289_845)
assert(easy([6, 6]) == 27_995_004)

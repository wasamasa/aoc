...
STRAIGHTS = 'abcdefghijklmnopqrstuvwxyz'.each_char.each_cons(3).map(&:join)

def first_requirement?(pw)
  STRAIGHTS.any? { |s| pw.include?(s) }
end

def second_requirement?(pw)
  !pw[/[iol]/]
end

def third_requirement?(pw)
  Set.new(pw.scan(/(.)\1/)).length >= 2
end

def valid_password?(pw)
  first_requirement?(pw) && second_requirement?(pw) && third_requirement?(pw)
end

def easy(input)
  loop do
    input = input.next
    return input if valid_password?(input)
  end
end
...
assert(easy('abcdefgh') == 'abcdffaa')
assert(easy('ghijklmn') == 'ghjaabcc')
pw = easy(input)
puts "easy(input): #{pw}"

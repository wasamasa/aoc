...
class PacketScanner
  def initialize(input, delay = 0)
    @layers = []
    @delay = delay
    input.each { |depth, range| @layers[depth] = range }
  end
  ...
  def caught?(i)
    return false unless @layers[i]
    ((@delay + i) % period(i)).zero?
  end

  def severity(i)
    return nil unless @layers[i]
    i * @layers[i]
  end
end
...
test_input = ['0: 3', '1: 2', '4: 4', '6: 4'].map { |line| parse(line) }
input = File.open('13.txt') { |f| f.readlines.map { |line| parse(line.chomp) } }

def easy(input)
  scanner = PacketScanner.new(input)
  severity = 0
  scanner.length.times do |i|
    severity += scanner.severity(i) if scanner.caught?(i)
  end
  severity
end

assert(easy(test_input) == 24)
...
def hard(input)
  delay = 0
  loop do
    return delay if scanner_pass?(input, delay)
    delay += 1
  end
end

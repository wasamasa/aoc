...
# the memory bank with the most blocks (ties won by the
...
# inserts one of the blocks. It continues doing this until it runs out
# of blocks; if it reaches the last memory bank, it wraps around to
# the first one.
...
input = File.open('06.txt') { |f| f.readline.split.map(&:to_i) }
test_input = [0, 2, 7, 0]
...
class Reallocator
  ...
  def redistribute
    ...
    @banks[index] = 0
    blocks.times do |i|
      @banks[(i + index + 1) % @banks.length] += 1
    end
    @steps += 1
  end
